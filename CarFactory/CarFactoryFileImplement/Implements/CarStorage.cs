...
namespace CarFactoryFileImplement.Implements
{
    public class CarStorage : ICarStorage
    {
        ...
        public CarViewModel GetElement(CarBindingModel model)
        {
            ...
            var car = source.Cars.FirstOrDefault(rec => rec.CarName == model.CarName || rec.Id == model.Id);
        ...
        private CarViewModel CreateModel(Car car)
        {
            return new CarViewModel
            {
                ...
                CarName = car.CarName,
                Price = car.Price,
                CarComponents = car.CarComponents.ToDictionary(recCC => recCC.Key, recCC =>
                    (source.Components.FirstOrDefault(recC => recC.Id == recCC.Key)?.ComponentName, recCC.Value))
            };
        }
    }
}
